...
module Simplifier.Externalize.FaceNormals (
  func
, reader
, writer
) where
...
func :: String
func = "HC_MSet_Face_Normals"

reader :: Maybe Key -> FilePath -> [Token]
reader (Just key) path = [
    TIdentifier "G_Read_Face_Normals"
  , TSymbol "("
  ] ++ asLookup key ++ [
    TSymbol ","
...
reader _ _ = error "FaceNormals.reader: Must be passed a `Just key'"
...
getData (stripSimpleDecls -> 
  (getPoints "normals" ->
    (normals, [hc| HC_MSet_Face_Normals(@k,@d | key offset _ |]))) 
      = (key, offset, reverse normals)
getData _ = error "VertexNormals.getData: Malformed HC_MSet_Face_Normals"
